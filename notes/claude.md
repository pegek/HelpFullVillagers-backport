...
| 9 | GUI (8 ekranów + GuiHandler) — stub | ✅(stub) | GuiHandler IGuiHandler; 8 stub klas (GuiScreen/GuiContainer); GuiTeachRecipe.VillagerContainerWorkbench extends ContainerWorkbench(manager,world,BlockPos); pełne UI later |
| 10 | Fish hook (EntityFishHookCustom + render + packet) — **risky, last** | 🔄(stub) | EntityFishHookCustom stub (konstruktory + Entity contract); RenderFishHookCustom stub; pełna logika/bobber TODO |
...
### ⬜ POZOSTAŁO (faza 2 — patrz notes/agent-handoff-phase2.md)
- **Pełne 8 GUI** (etap 9) — obecnie kompilowalne stuby bez logiki/rysowania. Referencje: `helpfulvillagers/decompiled_src/.../gui/`.
- **Pełny EntityFishHookCustom** (etap 10) — fizyka bobera, detekcja wody, logika połowu + RenderFishHookCustom. Najbardziej ryzykowny.
- (opcjonalnie) spawn eggs, dopracowanie flag z §8 (drzwi, flood-fill hal, addItem overflow).
...
- `EntityFishHook` 1.12.2 — największa niepewność (etap 10, wciąż stub).
